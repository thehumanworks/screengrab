...
- `audio.go` / `mac_audio.go` / `mac_audio_stub.go` / `mac_system_audio.go` / `mac_system_audio_stub.go` — opt-in audio lifecycle and artifact metadata, selected via `--audio mic|system|both` (`--microphone` stays as the `mic` alias). `audio.go` owns the shared `audioRecorder` wrapper over a `platformAudioTap`. Darwin records the default input as 16-bit PCM `audio.wav` with AVFoundation — tap buffers are deep-copied ("pinned") and written on a dedicated serial queue drained before close, and a mid-recording device change restarts the engine (converting formats if needed) instead of silently ending the take — and system audio as 16-bit PCM `system_audio.wav` via an SCStream with `capturesAudio` (window sources hear only the owning app via `initWithDisplay:includingApplications:` — a desktop-independent-window filter captures pixels but does not reliably produce audio; display sources hear the full mix, both under the existing Screen Recording permission — no new prompt). System-audio buffers are placed on a host-clock timeline with silence filling playback gaps and the tail, so the track spans the whole recording and a quiet source yields a valid silent track. Other platforms return explicit unsupported errors. Each track's clock is sampled at each successful frame acquisition so off-Space capture gaps remain aligned per track.
- `transcript.go` / `mac_transcript.go` / `mac_transcript_stub.go` / `wav.go` — versioned transcript documents, the darwin Speech-framework bridge, and the pure-Go WAV chunker. Recognition is post-capture and on-device only; unavailable locales preserve audio and frames and record the transcript status instead of falling back to a network service. On-device recognition emits a sequence of per-utterance final results (with timeline resets), so `SGSpeechCollector` accumulates every result until the task finishes, and tracks longer than 60s are split into bounded WAV chunks (`splitWAVForTranscription`) whose transcripts are re-merged on the full timeline — never trust a single `bestTranscription` for a long file.
...
7. **Audio (microphone and system) and transcription stay opt-in and locally associated.** Silent capture must not request audio permissions or write audio files. Audio-enabled output uses manifest v2 per-track frame offsets (`audio_offset_seconds`, `system_audio_offset_seconds`), owner-only artifact permissions, and on-device transcription per track; selected-frame exports retain the complete audio/transcript sidecars (`audio.wav`, `system_audio.wav`, `transcript.*`, `system_transcript.*`) and rewritten manifest. Keep the mic and system tracks as separate files — mixing them destroys the "what the user said" vs "what the machine played" distinction that AI consumers rely on.
