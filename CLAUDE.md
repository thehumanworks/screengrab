...
- `audio.go` / `mac_audio.go` / `mac_audio_stub.go` / `mac_system_audio.go` / `mac_system_audio_stub.go` — opt-in audio lifecycle and artifact metadata, selected via `--audio mic|system|both` (`--microphone` stays as the `mic` alias). `audio.go` owns the shared `audioRecorder` wrapper over a `platformAudioTap`. Darwin records the default input as 16-bit PCM `audio.wav` with AVFoundation, and system audio as 16-bit PCM `system_audio.wav` via an SCStream with `capturesAudio` on the same SCContentFilter shapes as capture (window sources hear only the owning app; display sources hear the full mix, both under the existing Screen Recording permission — no new prompt). Other platforms return explicit unsupported errors. Each track's clock is sampled at each successful frame acquisition so off-Space capture gaps remain aligned per track.
- `transcript.go` / `mac_transcript.go` / `mac_transcript_stub.go` — versioned transcript documents and the darwin Speech-framework bridge. Recognition is post-capture and on-device only; unavailable locales preserve audio and frames and record the transcript status instead of falling back to a network service.
