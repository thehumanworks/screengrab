...
- `audio.go` / `mac_audio.go` / `mac_audio_stub.go` — opt-in microphone lifecycle and artifact metadata. Darwin records the default input as 16-bit PCM `audio.wav` with AVFoundation; other platforms return an explicit unsupported error. Audio time is sampled at each successful frame acquisition so off-Space capture gaps remain aligned.
...
7. **Microphone and transcription stay opt-in and locally associated.** Silent capture must not request audio permissions or write audio files. Audio-enabled output uses manifest v2 frame/audio offsets, owner-only artifact permissions, and on-device transcription; selected-frame exports retain the complete audio/transcript sidecars and rewritten manifest.
